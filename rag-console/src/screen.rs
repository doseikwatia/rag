...
impl Screen {
    pub fn new(file: Option<File>) -> Self {
        ...
        Screen { file, text_buffer }
    }
    fn print_to_stdout(&mut self) {
        let skin = MadSkin::default_light();
        let (width, _) = terminal_size();
        let fmt_text = FmtText::from(&skin, &self.text_buffer, Some(width as usize));
        print!("{fmt_text}");
        let _ = stdout().flush();
    }
    ...
    pub fn read(&mut self, data:String)->String{
        ...
        self.write(data.as_bytes()).expect("unable to write read data");
        // stdout().flush().expect("unable to flush standard output");
        stdin().read_line(&mut usr_input).unwrap();
        let cleaned_input = usr_input.trim();
        self.write_str(cleaned_input).expect("unable to write cleaned input");
